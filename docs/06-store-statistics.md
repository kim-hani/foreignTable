...
**흐름 설명** — "내 위치 점을 만들고 → 그 점에서 일정 거리 안의 가게만 DB가 골라준다":

- **① 점 생성**: 요청의 위경도로 `ST_MakePoint(lng, lat)` 점을 만든다.
- **② 좌표계 지정**: `ST_SetSRID(.., 4326)`로 좌표계를 GPS 표준(WGS84)에 맞춘다.
- **③ 반경 필터**: `ST_DWithin(가게위치, 내위치, 반경, false)`이 두 점 거리가 반경 이내인 행만 반환.
- **속도 선택**: 마지막 인자 `false`는 정밀한 구면 계산을 생략해 속도를 높이는 선택.

**왜 DB에서 처리하나?**
모든 가게를 앱으로 가져와 거리 계산하면 비효율적. **PostGIS가 공간 인덱스로 DB에서 직접 필터링** → 빠르고 확장성 있음. 정밀 계산 필요 시 마지막 인자를 `true`로(use_spheroid). 만약 이 계산을 애플리케이션에서 했다면 전국 가게를 전부 메모리로 끌어와 일일이 거리를 재야 하지만, 공간 인덱스를 탄 DB는 "후보 자체를 좁혀서" 돌려주므로 데이터가 늘어도 안정적입니다.
...
**흐름 설명** — 매일 새벽 배치에서 가게별로 돌며, 순서가 곧 설계 의도다:

- **① 빈 168칸 초기화**: **7요일 × 24시간 = 168칸을 전부 0**으로 채운 빈 맵 생성 → 손님이 없던 시간대도 "0팀"으로 명시되어 프론트가 빈틈없는 그래프를 그릴 수 있음.
- **② DB 집계**: **DB가 직접** 최근 30일치를 요일·시간으로 `GROUP BY` 집계 (자바로 수천 건을 끌어오면 느리므로 PostgreSQL에 맡김).
- **③ 덮어쓰기·저장**: 집계 결과를 빈 맵의 해당 칸에만 덮어쓰고 JSONB로 저장 → 별도 `save()` 없이 **더티 체킹**이 처리.
